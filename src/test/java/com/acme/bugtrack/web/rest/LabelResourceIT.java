...
import static com.acme.bugtrack.domain.LabelAsserts.*;
...
import com.acme.bugtrack.repository.LabelRepository;
...
class LabelResourceIT {
    ...
    void createLabel() throws Exception {
        ...
        var returnedLabel = om.readValue(
            restLabelMockMvc
                .perform(post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(label)))
    ...
    void putWithMissingIdPathParamLabel() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restLabelMockMvc
            .perform(put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(label)))
    ...
    void patchWithMissingIdPathParamLabel() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restLabelMockMvc
            .perform(patch(ENTITY_API_URL).with(csrf()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(label)))
            .andExpect(status().isMethodNotAllowed());
